...
def get_tensor_from_camera(RT, Tquad=False):
    ...
    quad = rot.as_quat()[[3, 0, 1, 2]]  # expect wxyz format
    if Tquad:
...
def raw2outputs_nerf_color(raw, var, z_vals, rays_d, occupancy=False, device="cuda:0"):
    """
    Transforms model's predictions to semantically meaningful values.

    Args:
        raw (tensor, N_rays*N_samples*4): prediction from model.
        var (tensor, 1): depth variance.
    ...
    Returns:
        ...
        sensor_var (tensor, N_rays): aleatoric (deep) depth variance/uncertainty.
        model_var (tensor, N_rays): epistemic (model) depth variance/uncertainty.
    ...
    model_var = torch.sum(weights * tmp * tmp, dim=1)  # (N_rays)

    return depth_map, sensor_var, model_var, rgb_map, weights

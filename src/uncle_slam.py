...
class UncleSlam:
    ...
    def print_output_desc(self):
        """Print out the output folders for meshes, visualization, and checkpoints."""

        print(f"INFO: The output folder is {self.output}")

        if "Demo" in self.output:
            print(f"INFO: The GT, generated and residual depth/color images can be found under {self.output}/vis/")
        else:
            print(
                f"INFO: The GT, generated and residual depth/color images can be found under {self.output}/tracking_vis/ and {self.output}/mapping_vis/"
            )
    ...
    def run(self):
        ...
        processes = []
        for rank, process_function in enumerate([self.tracking, self.mapping]):
            process = mp.Process(target=process_function, args=(rank,))
            process.start()

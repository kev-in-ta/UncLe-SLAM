...
class Mesher(object):
    def __init__(self, cfg, args, slam, points_batch_size=500000, ray_batch_size=100000):
        ...
        Args:
            ...
            slam (class UncleSlam): UncleSlam main class.
        ...
        self.marching_cubes_bound = torch.from_numpy(np.array(cfg["mapping"]["marching_cubes_bound"]) * self.scale)
    ...
    def point_masks(
        ...
        forecast_mask_list = []
        unseen_mask_list = []
        for i, pnts in enumerate(torch.split(input_points, self.points_batch_size, dim=0)):
            ...
            if get_mask_use_all_frames:
                for i in range(0, idx + 1, 1):
                    ...
                    homo_points = torch.cat([points, ones], dim=1).reshape(-1, 4, 1).to(device).float()  # (N, 4)
                    # (N, 4, 1)=(4,4)*(N, 4, 1)
                    cam_cord_homo = w2c @ homo_points
                    cam_cord = cam_cord_homo[:, :3]  # (N, 3, 1)

                    K = torch.from_numpy(np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]]).reshape(3, 3)).to(
                        device
                    )
                    cam_cord[:, 0] *= -1
                    ...
                    edge = 0
                    cur_mask_seen = (
                        (uv[:, 0] < W - edge) & (uv[:, 0] > edge) & (uv[:, 1] < H - edge) & (uv[:, 1] > edge)
                    )
                ...
                for keyframe in keyframe_dict:
                    ...
                    if self.depth_test:
                        gt_depth = keyframe["gt_depth"].to(device).reshape(1, 1, H, W)
                        ...
                        # forecast
                        cur_mask_forecast = cur_mask_forecast.reshape(-1)
                        proj_depth_forecast = -cam_cord[cur_mask_forecast, 2].reshape(-1)
                        ...
                        max_depth = torch.max(keyframe["gt_depth"]) * 1.1
    ...
    def eval_points(self, p, decoders, c=None, stage="color", device="cuda:0"):
        ...
        for pi in p_split:
            ...
            pi = pi.unsqueeze(0)
            ret, var = decoders(pi, c_grid=c, stage=stage)
            ret = ret.squeeze(0)
    ...
    def get_grid_uniform(self, resolution):
        ...
        padding = 0.05
        x = np.linspace(bound[0][0] - padding, bound[0][1] + padding, resolution)
        y = np.linspace(bound[1][0] - padding, bound[1][1] + padding, resolution)
        z = np.linspace(bound[2][0] - padding, bound[2][1] + padding, resolution)
    ...
    def get_mesh(
        ...
        with torch.no_grad():
            ...
            if show_forecast:
                ...
                z_forecast = []
                for i, pnts in enumerate(torch.split(forecast_points, self.points_batch_size, dim=0)):
                    z_forecast.append(self.eval_points(pnts, decoders, c, "middle", device).cpu().numpy()[:, -1])
                z_forecast = np.concatenate(z_forecast, axis=0)
                z_forecast += 0.2

                z_seen = []
                for i, pnts in enumerate(torch.split(seen_points, self.points_batch_size, dim=0)):
                    z_seen.append(self.eval_points(pnts, decoders, c, "fine", device).cpu().numpy()[:, -1])
            ...
            if color:
                if self.color_mesh_extraction_method == "direct_point_query":
                    ...
                    z = []
                    for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                        z_color = self.eval_points(pnts.to(device).float(), decoders, c, "color", device).cpu()[..., :3]
                        z.append(z_color)
                    ...
                    sign = -1.0
                    length = 0.1
                    rays_o = torch.from_numpy(vertices + sign * length * vertex_normals).to(device)
                    color_list = []

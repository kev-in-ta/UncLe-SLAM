...
def check_proj(points, W, H, fx, fy, cx, cy, c2w):
    ...
    c2w[:3, 1] *= -1.0
    c2w[:3, 2] *= -1.0
    points = torch.from_numpy(points).to(device).clone()
    w2c = np.linalg.inv(c2w)
    w2c = torch.from_numpy(w2c).to(device).float()
    K = torch.from_numpy(np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]]).reshape(3, 3)).to(device)
...
def calc_2d_metric(rec_meshfile, gt_meshfile, align=True, n_imgs=1000):
    ...
    for i in range(n_imgs):
        ...
        if ours_depth[ours_depth > 0].shape[0] > 0:
